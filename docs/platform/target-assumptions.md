...
| Architecture | Docker/toolchain/build scripts assume ARMv7-A hard-float with NEON/VFPv4 and `arm-*-linux-gnueabihf`; htop also considers musl. | TARGET-CONFIGURABLE | Installed Launcher/loader confirm ELF32 little-endian ARM EABI5 hard-float, interpreter `/lib/ld-linux-armhf.so.3`, and glibc 2.30 loader identity. NEON/VFPv4 requirements and a compatible reproducible build toolchain remain UNKNOWN/unavailable. |
