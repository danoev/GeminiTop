...
| Device paths | `/dev/fb0`, `/dev/input`, `/application`, `/usr/local`, `/system`, `/apps`, and `/media/flash/nvm` are embedded throughout runtime/orchestrator code. | TARGET-CONFIGURABLE | Installed Stage-2 init confirms `/application`, `/usr/local`, `/media/flash/nvm`, `/dev/fb0`, and `/dev/input/event3` in stock configuration. This does not approve existing runtime code or prove every path. |
| Filesystem locations | Orchestrator constructs `PATH`/`LD_LIBRARY_PATH` from Sunplus/Roadtop directories; launcher and app code assume stock `Launcher` and application paths. | TARGET-CONFIGURABLE | CONFIRMED installed init puts NVM `bin`/`lib` first and declares `/application/bin/Launcher`; no W176 launcher action or persistent shadowing is enabled. |
...
| Architecture | Legacy Docker/toolchain/build scripts assume ARMv7-A hard-float with NEON/VFPv4 and `arm-*-linux-gnueabihf`; htop also considers musl. | TARGET-CONFIGURABLE | Installed Launcher/loader confirm ELF32 little-endian ARM EABI5 hard-float, ARMv7-A/VFPv3/NEON attributes, interpreter `/lib/ld-linux-armhf.so.3`, and glibc 2.30 loader identity. The isolated Stage-3 probe now has a reproducible pinned build using no feature above that installed evidence; physical custom-ELF loadability remains UNKNOWN. |
