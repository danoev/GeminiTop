...
def parse_probe_directory(path: Path) -> Dict[str, Any]:
    ...
    evidence.setdefault("identity.hardware", _first_match(cpuinfo, r"^Hardware\s*:\s*(.+)$"))
    evidence.setdefault("kernel.release", _first_match(uname, r"Linux\s+\S+\s+([^\s]+)"))
    nvm_hex = _first_match(mtd, r'^mtd\d+:\s+([0-9a-f]+)\s+[0-9a-f]+\s+"nvm"')
    if nvm_hex:
        evidence.setdefault("mtd.nvm_bytes", int(nvm_hex, 16))
    ...
    if partitions:
        ...
        evidence.setdefault("mtd.partition_signature", hashlib.sha256(canonical.encode("utf-8")).hexdigest())
    geometry = re.search(r"geometry\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)", fb)
    if geometry:
        evidence.setdefault("framebuffer.width", int(geometry.group(1)))
        evidence.setdefault("framebuffer.height", int(geometry.group(2)))
        evidence.setdefault("framebuffer.virtual_width", int(geometry.group(3)))
        evidence.setdefault("framebuffer.virtual_height", int(geometry.group(4)))
        evidence.setdefault("framebuffer.bits_per_pixel", int(geometry.group(5)))
    virtual_size = _first_match(fb, r"^virtual_size=(\d+,\d+)$")
    if virtual_size:
        virtual_width, virtual_height = (int(part) for part in virtual_size.split(",", 1))
        evidence.setdefault("framebuffer.virtual_width", virtual_width)
        evidence.setdefault("framebuffer.virtual_height", virtual_height)
    blocks = re.finditer(r'N:\s+Name="([^"]+)"(?:(?!\n\n).)*?H:\s+Handlers=([^\n]+)', inputs, re.DOTALL)
    for block in blocks:
        if "touch" in block.group(1).lower() or "fts" in block.group(1).lower():
            evidence.setdefault("touch.name", block.group(1))
            event = re.search(r"\bevent\d+\b", block.group(2))
            if event:
                evidence.setdefault("touch.event", event.group(0))
            break
    if usb_hash:
        evidence.setdefault("usb_handler.sha256", usb_hash.split()[0])
        handlers = []
        ...
        if handlers:
            evidence.setdefault("usb_handlers", handlers)
    if appinfo:
        evidence.setdefault("appinfo.present", True)
        if appinfo_hash:
            evidence.setdefault("appinfo.sha256", appinfo_hash.split()[0])
        clues = [clue for clue in ("GEMINI", "8368-XU") if clue.lower() in appinfo.lower()]
        if clues:
            evidence.setdefault("identity.product_clues", clues)
    names = sorted(
        {
            name
    ...
    if names:
        evidence.setdefault("applications", names)
    return {key: value for key, value in evidence.items() if value is not None and value != "UNKNOWN"}
