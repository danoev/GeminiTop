#!/bin/sh
# Derived from the independently reviewed Stage-2 removable-mount guard.
...
fail() {
    printf '%s\n' "w176-stage3: mount validation failed: $1" >&2
    exit 1
}
...
for RECORD in $MOUNT_RECORDS; do
    ...
    case "$DISK_NAME" in sd[a-z]*) ;; *) continue ;; esac
...
case "$CANDIDATE_FSTYPE" in vfat|msdos|fat) ;; *) fail "filesystem $CANDIDATE_FSTYPE is not approved" ;; esac
